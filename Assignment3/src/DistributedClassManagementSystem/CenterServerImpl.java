...
//CenterServerImpl class implements the WebService interface

@WebService(endpointInterface = "DistributedClassManagementSystem.CenterServer")
...
public class CenterServerImpl implements CenterServer {
	...
	public CenterServerImpl(final String name,final int serverPort,HashMap<String, Integer> nodePorts) {
		super();
	...
	public boolean transferRecord(String managerID, String recordID, String remoteCenterServerName) {
		...
		//Set lock on the record, in case if it is editing or transferring by another thread
		synchronized (record.getLock()) {
			
			//we have to find the record again to make sure the record has not been transfered and has not been removed from the hashmap by another thread 
			record = findRecord(recordID);
			if (record == null) {
				LoggerFactory.Log(this.name, String.format("Record not found, %s", recordID));
				return false;
	...
	private boolean transferRecord(final Record record, final String type, final String managerID, final int port, final String remoteServer) {
